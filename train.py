...
image_tensor_train, orig_img_tensor_train, annotation_tensor_train = inputs(FLAGS.dataset_train, train=True,
                                                                            batch_size=batch_size, num_epochs=1e4)
...
image_tensor, orig_img_tensor, annotation_tensor = tf.cond(is_training_placeholder,
                                                           true_fn=lambda: (image_tensor_train, orig_img_tensor_train,
                                                                            annotation_tensor_train),
                                                           false_fn=lambda: (image_tensor_val, orig_img_tensor_val,
                                                                             annotation_tensor_val))
...
# Define the model that we want to use -- specify to use 21 classes at the last layer
...
# get pool4 result and get prediction with it
...
# get bilinear upsample weights 2x
upsample_filter_np_x2_16s = bilinear_upsample_weights(2, number_of_classes)

upsample_filter_tensor_x2_16s = tf.Variable(upsample_filter_np_x2_16s, name='vgg_16/fc8/t_conv_x2_16s')

# Perform the upsampling to logits
upsampled_logits = tf.nn.conv2d_transpose(logits, upsample_filter_tensor_x2_16s,
                                          output_shape=tf.shape(aux_logits_16s),
                                          strides=[1, 2, 2, 1],
                                          padding='SAME')

# add aux 16s to logists
upsampled_logits = upsampled_logits + aux_logits_16s

# get pool3 result and get prediction with it
...
# get bilinear upsample weights 2x
upsample_filter_np_x2_8s = bilinear_upsample_weights(2, number_of_classes)

upsample_filter_tensor_x2_8s = tf.Variable(upsample_filter_np_x2_8s, name='vgg_16/fc8/t_conv_x2_8s')
...
# add aux 8s to logists
upsampled_logits = upsampled_logits + aux_logits_8s

# get bilinear upsample weights 8x
    ...
    for i in range(FLAGS.max_steps):
        ...
        if gs % 10 == 0:
            ...
            if gs % 200 == 0:
                ...
                cv2.imwrite(os.path.join(eval_folder, 'val_{0}_img.jpg'.format(gs)),
                            cv2.cvtColor(np.squeeze(val_orig_image), cv2.COLOR_RGB2BGR))
                cv2.imwrite(os.path.join(eval_folder, 'val_{0}_annotation.jpg'.format(gs)),
                            cv2.cvtColor(grayscale_to_voc_impl(np.squeeze(val_annot)), cv2.COLOR_RGB2BGR))
                cv2.imwrite(os.path.join(eval_folder, 'val_{0}_prediction.jpg'.format(gs)),
                            cv2.cvtColor(grayscale_to_voc_impl(np.squeeze(val_pred)), cv2.COLOR_RGB2BGR))

                crf_ed = perform_crf(val_orig_image, val_poss)
                cv2.imwrite(os.path.join(FLAGS.output_dir, 'eval', 'val_{0}_prediction_crfed.jpg'.format(gs)),
                            cv2.cvtColor(grayscale_to_voc_impl(np.squeeze(crf_ed)), cv2.COLOR_RGB2BGR))

                overlay = cv2.addWeighted(cv2.cvtColor(np.squeeze(val_orig_image), cv2.COLOR_RGB2BGR), 1,
                                          cv2.cvtColor(grayscale_to_voc_impl(np.squeeze(crf_ed)), cv2.COLOR_RGB2BGR),
                                          0.8, 0)
